var cpm = [
  ...
  function cpCalculate() {
    ...
    var maxCP = Math.floor(Math.max(10, ((baseAtk + atkValue) * Math.pow((baseDef + defValue), 0.5) * Math.pow((baseSta + staValue), 0.5) * Math.pow(cpm[lvlValue - 1], 2)) / 10));
  
    var nerfCP = Math.floor(Math.max(10, ((baseAtk + atkValue) * Math.pow((baseDef + defValue), 0.5) * Math.pow((baseSta + staValue), 0.5) * Math.pow(cpm[39], 2)) / 10));
  
    if (nerfCP >= 4000) {
      baseAtk = Math.round(baseAtk * 0.91);
      baseDef = Math.round(baseDef * 0.91);
      baseSta = Math.round(baseSta * 0.91);
      maxCP = Math.floor(Math.max(10, ((baseAtk + atkValue) * Math.pow((baseDef + defValue), 0.5) * Math.pow((baseSta + staValue), 0.5) * Math.pow(cpm[lvlValue - 1], 2)) / 10));
    }
